...
>(
  (
    ...
    <DialogPortal>
      ...
      >
        {children}
        <DialogPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground text-white dark:text-white">
          <X className="h-4 w-4" />
...
const DialogHeader = ({
  ...
  <div
    className={cn("flex flex-col .5 text-center sm:text-left", className)}
    {...props}
  />
);
